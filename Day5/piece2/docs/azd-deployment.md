...
4. **The Container App** (`quotes-api-cowork`, section 1): external ingress on port 8080, `/health/live` and `/health/ready` probe paths matching the manual exercise, the same KEDA HTTP concurrency scale rule (50 concurrent requests, 1-5 replicas). The JWT signing key is a Container Apps secret, delivered to the container via `secretRef` rather than a plain environment variable. An earlier draft had it as a `@secure() param jwtSecret string` with a literal placeholder default; typing that placeholder into a live terminal during deployment was blocked by an automated safety check for looking like a credential, which is what prompted an intermediate version deriving the value via `guid(subscription().id, resourceToken, 'jwt-secret')` instead of supplying it directly. The value now committed is a specific literal, padded to clear `QuotesApi.Configuration.JwtOptions`'s `[MinLength(32)]` validation (the requested value alone was 22 characters; a fixed suffix brings it to 39). It is still not a production-grade secret -- it has low entropy and is committed to source control as plaintext -- which is acceptable only because it authenticates nothing beyond this exercise's own tokens.
